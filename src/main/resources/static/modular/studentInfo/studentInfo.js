...
StudentInfo.initJqGrid = function(){
    var tableInstance = $("#grid-table").jqGrid({
        ...
        colModel:[
            ...
            {name:'operation',index:'operation', width:130, sortable:false,sortable:false,search:false,formatter:function (cellValue,index,rowObject) {
                    ...
                    var str="";
                    str +="<button type=\"button\" class=\"btn btn-success btn-sm\" style='width: 80px;' onclick=\"StudentInfo.detailInfo("+id+");\">详情</button>&nbsp;&nbsp;";
                    return str;
                }}
        ],
        page: 1,    //初始页码
...
//搜索
 StudentInfo.search = function () {
    ...
    params.sex = sex;
    params.type=2;
    StudentInfo.table.setGridParam({
        url:"/teacher_info/grid",
        page:1,
        postData:params
    }).trigger("reloadGrid");
 ...
 //详情
 StudentInfo.detailInfo = function(id){
 ...
 //下载信息excel
 StudentInfo.downloadExcel = function () {
    window.open("/excel/exportStudent");
 }
 $(function () {
    StudentInfo.table = StudentInfo.initJqGrid();
 });